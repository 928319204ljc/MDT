...
class Processor():
    ...
    def train(self, epoch, save_model=False):
        ...
        if len(all_labels) > 0:
            ...
            # 设定类别权重
            weights = np.array([6.0, 3.0, 1.0, 1.0])  # 假设按照 'Angry', 'Happy', 'Sad', 'Neutral' 的顺序
        ...
        if epoch == self.arg.num_epoch - 1:  # 仅在最后一个epoch生成
            ...
            features_3d = pca.fit_transform(all_outputs)

            # 绘制三维散点图，使用不同的颜色表示不同的类别
            label_names = ['Happy', 'Sad', 'Angry', 'Neutral']
            colors = ['cyan', 'black', 'magenta', 'yellow']
            for i, label_name in enumerate(label_names):
                indices = np.where(all_labels.flatten() == i)
                ax.scatter(features_3d[indices, 0], features_3d[indices, 1], features_3d[indices, 2], label=label_name,
                           alpha=0.5)

            ax.legend()
            plt.title('Scatter Plot of the Learned Deep Features - Training Set')
            plt.savefig(os.path.join(self.save_path, f'features_scatter_train_epoch_{epoch + 1}.png'))
            plt.close()
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        for ln in loader_name:
            ...
            if len(all_labels) > 0:
                # 确保为weights赋值
                weights = np.array([6.0, 1.0, 1.0, 0.5])  # 假设按照 'Angry', 'Happy', 'Sad', 'Neutral' 的顺序
                all_labels = np.array(all_labels)
                all_outputs = np.array(all_outputs)
