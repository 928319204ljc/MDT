...
class Adapter(nn.Module):
    def __init__(self,
                 ...
                 bottleneck=None,
                 dropout=0.0,
                 init_option="lora",  # 改成 "lora" 或其他已实现的初始化选项
                 adapter_scalar="1.0",
...
class SwinTransformerBlock3D(nn.Module):
    ...
    Args:
        ...
        adapter_cfg (dict, optional): Configuration dictionary for Adapter module.
    """

    def __init__(self, dim, num_heads, window_size=(2, 7, 7), shift_size=(0, 0, 0),
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, use_checkpoint=False, adapter_cfg=None):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        # 添加 Adapter 模块
        self.adapter1 = Adapter(config=adapter_cfg, d_model=dim, bottleneck=dim // 4)
        self.adapter2 = Adapter(config=adapter_cfg, d_model=dim, bottleneck=dim // 4)
    ...
    def forward_part2(self, x):
        ...
        return self.adapter2(x)  # 在 MLP 之后添加 Adapter
    ...
    def forward(self, x, mask_matrix):
        ...
        x = self.adapter1(x)  # 在注意力机制之后添加 Adapter
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
                 ...
                 adapter_cfg=None):  # 添加 adapter_cfg 参数
        super().__init__()
        self.window_size = window_size
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock3D(
                ...
                adapter_cfg=adapter_cfg  # 传递 adapter_cfg 参数
            )
    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            ...
            layer = BasicLayer(
                ...
                downsample=dfunc,
                use_checkpoint=use_checkpoint,
                adapter_cfg=adapter_cfg)  # 传递 adapter_cfg
...
class SpectralBlock(nn.Module):
    def __init__(self, channels, pseudo_depth=30, height=64, width=64):
        super(SpectralBlock, self).__init__()
        # 初始化第一个LayerNorm，对除batch维度以外的所有维度进行归一化
        self.norm1 = nn.LayerNorm([channels, pseudo_depth, height, width])
        self.freq_conv = nn.Conv3d(channels * 2, channels * 2, kernel_size=(1, 1, 1), padding=(0, 0, 0), bias=False)
        # 初始化第二个LayerNorm，处理经过频域转换和卷积后的维度
        self.norm2 = nn.LayerNorm([channels * 2, pseudo_depth, height, width])
        self.conv = nn.Conv3d(channels, channels, kernel_size=(1, 1, 1), padding=(0, 0, 0))
    ...
    def forward(self, x):
        ...
        # 将复数转换为实数和虚数部分，并将它们视为独立的通道
        x_freq_real = x_freq.real
        x_freq_imag = x_freq.imag
        x_freq_combined = torch.cat((x_freq_real, x_freq_imag), dim=1)  # 将实部和虚部合并到通道维度

        # 对频率域数据应用卷积
        x_conv = self.freq_conv(x_freq_combined)

        # 应用第二个LayerNorm于卷积输出
        x_conv = self.norm2(x_conv)

        # 将通道分开，并重建复数
        mid_channel = x_conv.shape[1] // 2
        x_conv_real = x_conv[:, :mid_channel, :, :, :]
        x_conv_imag = x_conv[:, mid_channel:, :, :, :]
        x_conv_complex = torch.complex(x_conv_real, x_conv_imag)

        # 应用IFFT
        x_time = torch.fft.ifft2(x_conv_complex, norm='ortho')

        # 添加额外的卷积层
        x_output = self.conv(x_time.real)
...
class SwinGait(BaseModel):
    ...
    def build_network(self, model_cfg):
        ...
        self.spectral_block1 = SpectralBlock(channels[0])  # 确保通道数与layer1的输出匹配
    ...
    def get_optimizer(self, optimizer_cfg):
        ...
        params_list = [
            {'params': [p for n, p in transformer_params if any(nd in n for nd in transformer_no_decay)],
             'lr': optimizer_cfg['lr'], 'weight_decay': 0.},
            ...
            {'params': self.FCs.parameters(), 'lr': optimizer_cfg['lr'] * 0.1,
             'weight_decay': optimizer_cfg['weight_decay']},
            {'params': self.BNNecks.parameters(), 'lr': optimizer_cfg['lr'] * 0.1,
             'weight_decay': optimizer_cfg['weight_decay']},
        ]
        for i in range(5):
            if hasattr(self, 'layer%d' % i):
                params_list.append(
                    {'params': getattr(self, 'layer%d' % i).parameters(), 'lr': optimizer_cfg['lr'] * 0.1,
                     'weight_decay': optimizer_cfg['weight_decay']}
                )
    ...
    def forward(self, inputs):
        ...
        out1 = self.spectral_block1(out1)  # 在layer1后添加Spectral Block
